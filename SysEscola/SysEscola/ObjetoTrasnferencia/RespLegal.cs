using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace ObjetoTrasnferencia
{
    public class RespLegal
    {
        public int Cod_rep_legal { get; set; }
        public string NomeRep_Legal { get; set; }
        public DateTime Data_Nascto { get; set; }
        public int Cod_Est_Civil { get; set; }
        public string End_Rua { get; set; }
        public string End_Nr { get; set; }
        public string End_Comp { get; set; }
        public int Cod_Bairro { get; set; }
        public string End_Cep { get; set; }
        public string End_Cidade { get; set; }
        public string UF_Residencia { get; set; }
        public string Tel_Residencial { get; set; }
        public string Celular { get; set; }
        public string Telefone_Favor { get; set; }
        public string End_Referencia { get; set; }
        public string Cart_Ident { get; set; }
        public string Org_Exped { get; set; }
        public string CPF { get; set; }
        public string Naturalidade { get; set; }
        public string Nacionalidade { get; set; }
        public string UF_Naturalidade { get; set; }
        public int Cod_Escolaridade { get; set; }
        public string Condicao { get; set; }
        public string Profissao { get; set; }
    }
}
